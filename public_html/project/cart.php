    ...
    switch ($action) {
        ...
        case "update":
            ...
            try {
                ...
                $query="DELETE FROM Cart WHERE id = :cid and user_id = :uid and desired_quantity <=0 ";
                $stmt = $db->prepare($query);
                $stmt->bindValue(":cid", se($_POST, "cart_id", 0, false), PDO::PARAM_INT);
                $stmt->bindValue(":uid", get_user_id(), PDO::PARAM_INT);
        ...
        case "delete":
            $query="DELETE FROM Cart WHERE id = :cid and user_id = :uid ";
                ...
                $stmt->bindValue(":uid", get_user_id(), PDO::PARAM_INT);
                $stmt->execute();
            error_log(var_export($_POST, true)); // i have implemented this one
            flash("Cart is Cleared", "warning");
            break;
    }
}
