...
if (isset($item_id) && $quantity > 0) {
    if (is_logged_in()) {
        $db = getDB();
        $db->beginTransaction();
        $stmt = $db->prepare("INSERT INTO Orders (item_id, quantity, user_id) VALUES(:iid, :q, :uid) 
        ...
        try {
            ...
            $stmt = $db->prepare("UPDATE OrdersTable SET quantity = quantity - :q WHERE user_id = :uid AND item_id = :iid");
            try {
                $stmt->bindValue(":iid", $item_id, PDO::PARAM_INT);
                $stmt->bindValue(":q", 1/*hard coding 1 for now*/, PDO::PARAM_INT);
                $stmt->bindValue(":uid", get_user_id(), PDO::PARAM_INT);
